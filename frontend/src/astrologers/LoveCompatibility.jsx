...
const LoveCompatibility = ({ openPaymentModal }) => {
  ...
  const confirmCompatibilityUnlock = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setCurrentStep(1); // Start at step 1 for new report
  ...
  const handlePaymentRedirect = () => {
    ...
    if (openPaymentModal) {
      openPaymentModal(); // Call the function to open Navbar's payment modal
    } else {
      navigate("/wallet/add-credits"); // Use navigate instead of window.location.href
    }
  };
  ...
  const renderCompatibility = () => {
    ...
    const compatibilityScoreMatch = narrative.match(/compatibility score of (\d+)%/);
    const compatibilityScore = compatibilityScoreMatch ? parseInt(compatibilityScoreMatch[1]) : 50;
...
🔮 Ontdek meer of verbind je met een AI Love Psychic om jullie kosmische liefde tot leven te brengen!        </p>
                  </div>
